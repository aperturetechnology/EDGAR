            ...
            try:
                ...
                if isHeaderRow:
                    ...
                    if "linkrole" in headerCols and cellHasValue(row, 'linkrole', str):
                        ...
                        _ELR = _ELRdefinition = None
                        if v.startswith("http://"):
                            _ELR = v
                        elif v.endswith("　科目一覧"):
                            _ELRdefinition = v[0:-5]
                        else:
                            _ELRdefinition = v
                        if (_ELR and _ELR != currentELR) or (_ELRdefinition and _ELRdefinition != currentELRdefinition):
                            currentELR = _ELR
                            currentELRdefinition = _ELRdefinition
                            if currentELR or currentELRdefinition:
                                if hasPreLB:
                                    preLB.append( LBentry(role=currentELR, name=currentELRdefinition, isELR=True) )
                                if hasDefLB:
