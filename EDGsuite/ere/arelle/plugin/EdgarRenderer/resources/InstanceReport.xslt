...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="InstanceReport">
    ...
    <xsl:variable name="isOuterReport" select="generate-id(.) = generate-id(/InstanceReport)"/>
      ...
      <xsl:if test="$displayLabelColumn">
        ...
        <th class="tl">
          <xsl:attribute name="colspan">
            <xsl:value-of select="1 + ($hasLabelFootnotes > 0)"/>
          </xsl:attribute>
          <xsl:choose>
          ...
          <xsl:for-each select="Labels/Label[not(contains(@Label, ' Ended'))]">
            <div>
              ...
              <xsl:if test="not(position() = last())">
                <xsl:value-of select="../../LabelSeparator"/>
              </xsl:if>
            </div>
          </xsl:for-each>
        </xsl:otherwise>
  ...
  <xsl:template name="reportBreak">
    ...
    <tr>
      <td colspan="{ $cols + $idxs + 1 }" style="height: 1em;">
        <hr/>
      </td>
    </tr>
  </xsl:template>
  ...
  <xsl:template name="authRefLink">
    <xsl:choose>
      ...
      <xsl:when test="not(../../../IsTransposed = 'true') and $majorversion > 2 and string-length(ElementName) = 0">
  ...
  <xsl:template name="authRefData">
    ...
    <xsl:variable name="tmp">
      <xsl:choose>
        ...
        <xsl:when test="string-length(ElementDefenition) > 0 and not(ElementDefenition = 'No definition available.')"> ElementDefenition </xsl:when>
        <xsl:when test="string-length(ElementReferences) > 0 and not(ElementReferences = 'No authoritative reference available.')"> ElementReferences </xsl:when>
          ...
          <div class="body" style="padding: 2px;">
            <xsl:if test="string-length(ElementDefenition) > 0 and not(ElementDefenition = 'No definition available.')">
              ...
              <div>
                ...
                <p>
                  <xsl:value-of select="ElementDefenition"/>
                </p>
              </div>
            </xsl:if>
          ...
          <xsl:when test="IsNumeric = 'false'">
            <xsl:choose>
              <xsl:when test="DisplayDateInUSFormat = 'true'">
                ...
                <xsl:if test="not(NonNumbericText = '')">
          ...
          <xsl:when test="IsNumeric = 'true'">
            <xsl:choose>
              <xsl:when test="DisplayZeroAsNone = 'true' and NumericAmount = 0">
                <xsl:attribute name="class">nump</xsl:attribute> none </xsl:when>
              <xsl:otherwise>
  ...
  <xsl:template name="dateFilters">
    ...
    <xsl:text> </xsl:text>
    <xsl:choose>
      <xsl:when test="string-length($day) = 1"> 0<xsl:value-of select="$day"/>, <xsl:text> </xsl:text>
      </xsl:when>
      <xsl:otherwise>
  ...
  <xsl:template name="textFilters">
    <xsl:choose>
      <xsl:when test="not(NonNumbericText = '')">
        <xsl:choose>
